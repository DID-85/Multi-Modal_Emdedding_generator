...
from twelvelabs.exceptions import TwelveLabsError
import logging
import os
import time
# Configure logging
...
client = TwelveLabs(api_key="tlk_3K8XWSE1TXAKFT2A14A140SC7YRY")
...
@app.post("/create-video-embedding/")
async def create_video_embedding(video_file: UploadFile = File(...)):
    """
    Endpoint to upload a video file, create a video embedding task, and return the embeddings.
    """
    try:
        ...
        if not video_file.filename.lower().endswith((".mp4", ".mov", ".avi", ".mkv")):
            raise HTTPException(
                status_code=400,
                detail="Invalid file type. Supported formats: .mp4, .mov, .avi, .mkv"
            )
        ...
        # Create the video embedding task using the local file
        ...
        while True:
            task_status = client.embed.task.retrieve(task.id)
        ...
        if task.video_embedding and task.video_embedding.segments:
            for segment in task.video_embedding.segments:
                embeddings.append({
                    ...
                    "embeddings": segment.embeddings_float
                })

        return JSONResponse(content={"task_id": task.id, "embeddings": embeddings})
